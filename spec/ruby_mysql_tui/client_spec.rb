...
RSpec.describe RubyMysqlTui::Client, '#create_table (custom columns)' do
  ...
  it 'creates a table with custom columns' do
    table_name = 'test_table'
    columns = [{ name: 'name', type: 'VARCHAR(255) NULL' }, { name: 'email', type: 'VARCHAR(255) NOT NULL' }]
    sql = "CREATE TABLE `#{table_name}` (id INT PRIMARY KEY AUTO_INCREMENT, `name` VARCHAR(255) NULL, `email` VARCHAR(255) NOT NULL)"
    expect(mock_mysql_client).to receive(:query).with(sql)
