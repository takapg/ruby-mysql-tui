...
RSpec.describe RubyMysqlTui::InputHandler::TableManager do
  let(:client) { instance_double('RubyMysqlTui::Client') }
  let(:prompt) { instance_double('TTY::Prompt') }
  let(:state) { { items: ['t1', 't2'], selected_index: 0, selected_db: 'test_db' } }

  describe '.handle_create_table' do
    it 'creates a table when a valid name is provided' do
      ...
      expect(client).to receive(:list_tables).with('test_db').and_return(['t1', 't2', 'new_table'])

      result = described_class.handle_create_table(state, client, prompt)
      expect(result[:items]).to eq(['t1', 't2', 'new_table'])
    end
