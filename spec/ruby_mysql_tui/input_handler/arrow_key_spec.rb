...
RSpec.describe RubyMysqlTui::InputHandler do
  let(:state) { { focus: :right, view_mode: :records, selected_table: 'test_table', records: [{ 'id' => 1 }] } }
  ...
  describe '.handle_input' do
    context 'when arrow key escape sequences are provided' do
      it 'calls handle_up for "\e[A" and "\eOA"' do
        expect(RubyMysqlTui::InputHandler::ScrollHandler).to receive(:handle_up).with(state, client).and_return(state)
      ...
      it 'calls handle_down for "\e[B" and "\eOB"' do
        expect(RubyMysqlTui::InputHandler::ScrollHandler).to receive(:handle_down).with(state, client).and_return(state)
