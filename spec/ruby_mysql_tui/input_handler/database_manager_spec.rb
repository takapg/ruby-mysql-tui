...
RSpec.describe RubyMysqlTui::InputHandler::DatabaseManager, '.handle_create_database' do
  let(:client) { instance_double('RubyMysqlTui::Client') }
  let(:prompt) { instance_double('TTY::Prompt') }
  let(:state) { { items: %w[db1 db2], selected_index: 0 } }
  ...
  it 'creates a database when a valid name is provided' do
    ...
    expect(client).to receive(:list_databases).and_return(%w[db1 db2 new_db])

    result = described_class.handle_create_database(state, client, prompt)
    expect(result[:items]).to eq(%w[db1 db2 new_db])
  end
...
RSpec.describe RubyMysqlTui::InputHandler::DatabaseManager, '.handle_drop_database' do
  let(:client) { instance_double('RubyMysqlTui::Client') }
  let(:prompt) { instance_double('TTY::Prompt') }
  let(:state) { { items: %w[db1 db2], selected_index: 0 } }
  ...
  it 'drops a database when confirmed' do
    ...
    expect(client).to receive(:list_databases).and_return(%w[db2])

    result = described_class.handle_drop_database(state, client, prompt)
    expect(result[:items]).to eq(%w[db2])
