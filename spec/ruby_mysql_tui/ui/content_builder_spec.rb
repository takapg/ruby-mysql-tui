...
RSpec.describe RubyMysqlTui::UI::RecordsContentBuilder, 'filtering basic - results' do
  ...
  it '渡されたレコードセットをそのまま表示すること' do
    filtered_records = [{ 'id' => 1, 'name' => 'Alice' }]
    state = { records: filtered_records, records_filter_query: 'al', selected_table: 'test' }
    output = described_class.build_view(state, width, height)
    expect(output).to include('Alice')
    expect(output).not_to include('Bob', 'Charlie')
  end

  it 'レコードセットが空の場合、「No records found」と表示すること' do
    state = { records: [], records_filter_query: 'nonexistent', selected_table: 'test' }
    output = described_class.build_view(state, width, height)
...
RSpec.describe RubyMysqlTui::UI::RecordsContentBuilder, 'filtering advanced - column search' do
  ...
  it 'フィルタリング済みのレコードセットが正しく表示されること' do
    filtered_records = [{ 'id' => '102', 'name' => 'Bob' }]
    state = { records: filtered_records, records_filter_query: '102', selected_table: 'test' }
    output = described_class.build_view(state, width, height)
...
RSpec.describe RubyMysqlTui::UI::RecordsContentBuilder, 'filtering advanced - clamp' do
  ...
  it 'レコードセットのサイズに基づいて selected_record_index が clamp されること' do
    filtered_records = [{ 'id' => 1, 'name' => 'Alice' }]
    state = {
...
RSpec.describe RubyMysqlTui::UI::RecordsContentBuilder, 'header with total records' do
  let(:width) { 100 }
  let(:height) { 20 }
